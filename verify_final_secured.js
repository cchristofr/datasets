...
async function runTest(page, config) {
    ...
    const buttonSelector = `#sirene-btn-${config.rowIndex}`;

    // Enter API Key AND trigger the keyup event to enable the button
    await page.type('#api-key', 'mock-api-key');

    // Wait for the button to become enabled
    await page.waitForFunction((selector) => !document.querySelector(selector).disabled, buttonSelector, { timeout: 5000 });
    console.log('SIRENE button is enabled as expected.');
...
(async () => {
    ...
    try {
        ...
        await runTest(page, {
            testName: 'Supplier_Success_Secured', htmlFile: 'index.html', testFile: 'dummy_sirene_api_test.xlsx',
        ...
        await runTest(page, {
            testName: 'Client_Success_Secured', htmlFile: 'index_clients.html', testFile: 'dummy_sirene_api_test.xlsx',
        ...
        await runTest(page, {
            testName: 'Supplier_Errors_Secured', htmlFile: 'index.html', testFile: 'dummy_sirene_error_test.xlsx',
        ...
        await runTest(page, {
            testName: 'Client_NetworkError_Secured', htmlFile: 'index_clients.html', testFile: 'dummy_sirene_error_test.xlsx',
